...
1. A hosted version of a React app (or similar) on GitHub Pages that is located in the `namallium` organization.
...
Once these requirements are in place, to use the embed, add the URL to "Embed Code or URL" in the format: `https://namallium.github.io/yale-po-filter-app/assets` where `assets` is a directory where `app.js` and `app.css` reside.
